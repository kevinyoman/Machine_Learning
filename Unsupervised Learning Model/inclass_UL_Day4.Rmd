...
- `dimensi`: kolom/variabel
- `informasi`: variance

2. Apakah nilai dari variansi dipengaruhi oleh skala dari nilai-nya? Iyaa
...
- Dibuat sumbu baru PC1 dan PC2 untuk menangkap informasi dari X1 dan X2 (variabel awal).
...
> Kesimpulan: Ketika kita ambil PC1 maka sudah mendapat 90% informasi data, dan mengurangi dimensi 50%. 
...
* mutate_at : untuk mengubah tipe data dari variabel tertentu, menjadi tipe data lain
* mutatei_if : mengubah data dari tipe data tertentu, tipe data baru
```{r warning=FALSE}
library(lubridate)
...
```
Insight:
- Skala data yang diiimiliki itu berbeda,
...
[X] Tujuannya untuk membuat rekomendasi.
[ ] Model dapat dievaluasi dengan data test.
[ ] Digunakan untuk menyelesaikan permasalahan regresi dan klasifikasi.
...
[ ] Model tidak dapat dilakukan evaluasi, karena tidak memiliki "ground truth" atau label aktual.
[ ] Digunakan untuk mengidentifikasi kemungkinan untuk melakukan reduksi dimensi, clustering, atau untuk menemukan pola anomali atau yang tidak sesuai dengan data/pengamatan lainnya.
[X] Tujuannya untuk membuat prediksi.
...
[ ] Bisa menjadi salah satu teknik untuk improve model, namun tidak selalu menjadi lebih baik.
[ ] Mengurangi resiko terjadinya multikolinearitas, karena nilai antar PC sudah tidak saling berkorelasi.
[X] Model mudah diinterpretasikan, karena nilai PC merupakan campuran dari beberapa variabel.
...
biplot(x = ppt_100_pca,
       cex = .8, # mengatur font size pada biplot
...
Obs 51 di-indikasikan sebagai outlier pada variable Sale.Prices
...
Obs 96 di-indikasikan sebagai outlier pada variable Total.Units

```{r}
summary(ppt_100)
```

Catatan: Selain membandingkan dengan nilai lainnya, kita juga bisa memastikan apakah sebuah obs termasuk outlier atau tidak berdasarkan nilai mean/median. (Credit: Ka Dicky)
...
```{r}
cor(ppt_100$RESIDENTIAL.UNITS,ppt_100$COMMERCIAL.UNITS)
```
...
fancy_biplot(x = "PC3", y = "PC4", pca_crime)
```

Insight dari biplot:

1. Perhatikan negara bagian Arizona, Colorado, Illinois, dan Texas berada. Negara bagian mana lagi yang paling dekat dengan negara-negara tersebut? New York
...
- hapus kolom `verification_status`, `grdCtoA`.
...
Notes:
ncp = 10 karena jumlah variabel numeric pada loan_clean adalah 7 (jumlah quantivar), tujuannya adalah kita bisa akses seluruh hasil PC.
...
Sepuluh outlier terluar: 43, 351, 368, 512, 749, 1146, 1185, 1228, 1310, dan 1432.
...
- Pasangan variabel yang saling berkorelasi tinggi positif: annual_inc dan log_incc
- Pasangan variabel yang saling berkorelasi tinggi negatif: dti dan log_inc
- Pasangan variabel yang hampir tidak saling berkorelasi: dti dan revol_bal
...
loan_keep <- as.data.frame(loan_pca$ind$coord)
...
[X] Mengetahui besar nilai dari observasi.
...
[X] Melihat besar hubungan observasi terhadap PCnya.
...
[X] 90 derajat = berkorelasi kuat
[ ] mendekati 180 derajat = berkorelasi negatif

5. Berdasarkan Letak titik observasi terhadap arah panah, berikut adalah pertanyaan yang benar:
[X] Observasi yang searah panah dan letaknya jauh dari pusat data (titik nol) maka memiliki nilai yang besar di variable panah tersebut.
[X] Observasi yang berlawanan arah panah, maka nilainya kecil di variable tersebut.
...
```{r}
# reconstruct data menggunakan seluruh PC: 1-10
loan_all_reconst <- reconst(loan_pca,
                            ncp = 10)
...
Pengelompokan -> memisahkan menjadi beberapa kelompok untuk data kita

- Observasi di cluster/kelompok yang memiliki karakteristik semirip mungkin 
  * Contoh: Data pengelompok data pasien (1 variable golongan darah)
    ~ Kelompok 1: Teridiri dari pasien yang memiliki gol.darah A
    ~ Kelompok 2: Teridiri dari pasien yang memiliki gol.darah B

- Observasi dari cluster/kelompok berbeda memiliki karakteristik yang sebeda mungkin 
  * Contoh: Data pengelompok data pasien (1 variable golongan darah)
    ~ Kelompok 1: Teridiri dari pasien yang memiliki gol.darah A
    ~ Kelompok 2: Teridiri dari pasien yang memiliki gol.darah B
    ~ Kelompok 3: Teridiri dari pasien yang memiliki gol.darah O
    ~ Kelompok 4 Teridiri dari pasien yang memiliki gol.darah AB
    
Contoh Hasil Clustering
    
Data Loan (Variable: Grade, Purpose, Verification)    

- Cluster 1: Grade A - Debt Consolidation, Credicard Payment - Not Verified 
- Cluster 2: Grade B - Housing - Verified
- Cluster 3: Grade C - Startup - Verified
...
- *Within Sum of Squares* (`$withinss`): jumlah jarak kuadrat dari tiap observasi ke centroid tiap cluster.
  ~ Harapannya nilai dari WSS nilainya sekecil mungkin
  ~ Karena jika nilai kecil, berarti jarak antar setiap observasi pada cluster tersebut ke titik centroidsnya akan semakin dekat (datanya akan semakin sama karakteristiknya)
...
- *Between Sum of Squares* (`$betweenss`): jumlah jarak kuadrat terbobot dari tiap centroid ke rata-rata global. Bobotnya berdasarkan banyaknya observasi pada cluster.
  ~ Harapannya nilai dari BSS sebesar mungkin
  ~ Karena semakin besar nilainya, semakin jauh jarak antar tiap centorid yang ada. (Datanya akan semakin berbeda karakteristiknya)

- *Total Sum of Squares* (`$totss`): jumlah jarak kuadrat dari tiap observasi ke rata-rata global.
  ~ Nilai TSS tidak bisa berdiri sendiri, melainkan akan disandingkan dengan nilai BSS
  ~ Rumusnya: BSS/TSS
  ~ Harapannya perbandingan antara nilai BSS/TSS mendekati 1.
  
Sebagai contoh nilai BSS = 500, agar perbandingan antar BSS/TSS mendekati, berapa nilai TSS?
...
1. Kebutuhan dari segi bisnis, data dibutuhkan menjadi berapa kelompok; atau
  Contoh: Misalkan kita bekerja di sebuah Bank, dan kita diminta untuk mengelompokan data nasabah kita menjadi ke dua kelompok, Nasabah Prioritas & Nasaban Tidak Prioritas.
...
Nilai K yang akan kita coba manfaatkan adalah 4, berarti hasil akhir dari clustering kita akan sebanyak 4 kelompok.

Catatan: 

- Kita bisa memilih nilai K yang paling "optimum" dengan melihat penurunan WSSnya.
- Untuk hasil nilai WSSnya akan selalu menurunun seiring bertambahnya nilai K.
...
3. Letak pusat cluster/centroid, biasa digunakan untuk profiling cluster (akan dibahas lebih dalam di bagian akhir)
...
# benchmark untuk update model, recall / precision +- 10%
# rpubs.com/VicNP/cust-num-cat
```
...
# apply(whisky_centroid$Body, 1, max, na.rm=T)

whisky_centroid %>% 
  pivot_longer(-kelompok) %>% 
  group_by(name) %>% 
  summarise(cluster_min=which.min(value), cluster_max=which.max(value))
```
```{r}
whisky_clean %>% filter(kelompok == 2)
...
**Cluster 1**:
  - Paling tinggi di cita rasa: Body
  - Paling rendah di cita rasa: Tobacco
  - Label: _____

**Cluster 2**:
  - Paling tinggi di cita rasa: Body, Smoky
  - Paling rendah di cita rasa: Tobacco
  - Label: _____

**Cluster 3**:
  - Paling tinggi di cita rasa: Sweetness
  - Paling rendah di cita rasa: Tobacco
  - Label: _____

**Cluster 4**:
  - Paling tinggi di cita rasa: Smoky
  - Paling rendah di cita rasa: Tobacco
  - Label: _____
...
```
```{r}
sum(whisky_kmeans$withinss)
whisky_kmeans$betweenss
whisky_kmeans$betweenss / whisky_kmeans$totss

sum(whisky_kmeans2$withinss)
whisky_kmeans2$betweenss
whisky_kmeans2$betweenss /whisky_kmeans2$totss
```

Additional: k-means = 3
```{r}
RNGkind(sample.kind = "Rounding")
set.seed(100)
