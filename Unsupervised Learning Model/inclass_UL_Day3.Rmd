...
- Observasi di cluster yang sama memiliki karakteristik semirip mungkin 
...
whisky_clean <- whisky %>% select(-c(RowID,Postcode,Latitude,Longitude))
...
whisky_clean <- whisky_clean %>% select(-Tobacco)
```
...
fviz_nbclust(x = whisky_clean, 
             FUNcluster = kmeans,
             method = 'wss')
# dipilih perubahan landai yang tidak signifikan dan ambil k belakang
```
...
# cek variable yang paling berkontribusi di setiap cluster
```
...
whisky_centroid <- whisky %>% 
  group_by(... = ) %>% # melakukan grouping pada setiap cluster
  summarise_all(...) # dihitung rata2nya pada tiap cluster
...
fviz_cluster(object = ..., # object kmeans
             data = ...) # data variable numerik
```
