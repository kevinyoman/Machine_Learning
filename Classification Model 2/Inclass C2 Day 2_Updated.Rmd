...
Model Naive dengan 400 data yang sudah kita miliki:
-Acc: 80%

Ketika kita memliki 400 data baru, dan kita coba cek hasil confusion matrixnya dengan model yang ada
- Acc: 60% 
...
- Kelas positif: Spam karena SMS spam itu lebih penting untuk dideteksi dibandingkan dengan SMS ham
- Kelas negatif: Ham
...
- Adakah kolom yang tipe datanya perlu disesuaikan? `v1` -> Tipe Data Factor
...
saya sedang menuju  algoritma  bersama mas kevin
saya sedang menuju algoritma bersama mas kevin
...
Ketika kita ingin memilih salah satu nilai metriks, perhatikan FP & FN
  - False Positive: Diprediksi SPAM tapi Aktualnya HAM 
  - False Negative: Diprediksti HAM tapi Aktualnya SPAM

Rumus Precsion: TP / (TP + FP)
  - Jika kita ingin nilai Presisi kita tinggi, kita harus mengurangi FP
  - Jika nilai FP kita tekan, maka kondisi email HAM dianggap SPAM semakin sedikit

Rumus Recall: TP / (TP + FN)
  - Jika kita ingin nilai Recall tinggi, kita harus mengurangi FN
  - Jika nilai FN kita tekan, makan kondisi email SPAM dianggap HAM semakin sedikit
...
summary(data_train$glucose)
plot(data_train$diabetes,data_train$glucose)
```
...
diab_train_up <- upSample(diab_train %>% select(-diabetes),
                          diab_train$diabetes, yname='diabetes')

diab_train_down <- downSample(diab_train %>% select(-diabetes),
                          diab_train$diabetes, yname='diabetes')
```
...
```{r}
# Please type your code down below
model_dt <- ctree(diabetes~., data_train )

model_diabetes <- ctree(diabetes~., diab_train_up )
...
train_pred <- predict(object = model_dt, 
                      newdata = data_train, 
                      type = "response")

# prediksi ke data test
test_pred <- predict(object = model_dt, 
                     newdata = data_test, 
                     type = "response")
```
...
confusionMatrix(data=train_pred, reference=data_train$diabetes, positive="pos")
```
...
confusionMatrix(data=test_pred, reference=data_test$diabetes, positive="pos")
