...
Ketika data tidak balance (kelas 0 & 1), Accuracy tidak dapat dijadikan sebagai tolak ukur
...
- Recall tinggi, lebih dibutuhkan oleh customer
...
- Tidak mau ada prediksi kelas positif yang salah
-Precision tinggi, lebih dibutuhkan oleh perusahaan
...
loans.test$pred.Label <- ifelse(loans.test$pred.Risk > 0.4, 1, 0)
loans.test$pred.Label <- as.factor(loans.test$pred.Label)
# confusion matrix
confusionMatrix(loans.test$pred.Label, loans.test$not_paid, positive = '1')
```
...
Insight: .data masih balance, dibawah 70 30
...
idx <- sample(nrow(wbcd_clean),nrow(wbcd_clean)*0.8)

wbcd_train <- wbcd_clean[idx,]
wbcd_test <- wbcd_clean[-idx,]
...
Insight: proporsi masih termasuk balance. Bila di balancekan, hasil model tidak berpengaruh banyak
