...
```{r}
# your code here
model_crime <- lm(crime_rate~police_exp60+time_prison, crime)
summary(model_crime)
```
___
...
4. Which of the following best describes the slope for `time_prison`?
  ...
  - [X] It's a positive slope, and is statistically insignificant (P-value higher than 0.05)
...
crime_test
library(MLmetrics)
pred_model <- predict(model_step,crime_test)
round(MAE(y_pred=pred_model,y_true=crime_test$crime_rate),2)
```
___
