...
**Summary & Knowledge Check Day 3:**

* Evaluasi model:
  + diinginkan error kecil
  + ragam metrics/nilai error: 
    - MAE : bisa diinterpretasikan, tidak sensitif terhadap outlier
    - RMSE : bisa diinterpretasikan, sensitif terhadap outlier
    - MSE : tidak bisa diinterpretasikan, sensitif terhadap outlier, digunakan untuk membandingkan model untuk keperluan pribadi
  + pilih 1 metrics error, kemudian dibandingkan error dari masing-masing model. pilih model yang menghasilkan error terkecil.
  
1. Berikut yang BUKAN code Multiple Linear Regression adalah ...
[ ] lm(Profit ~ Sales + Discount, data) -> 2 var prediktor
[x] lm(Profit ~ 1, data) -> 0 var prediktor
[ ] lm(Profit ~ . , data) -> semua var prediktor

2. Berikut pernyataan yang TIDAK TEPAT terkait pemilihan variable pada Multiple Linear Regression ...
[ ] Dapat menggunakan pengetahuan/pertimbangan bisnis -> biasanya ini jadi pertimbangan paling awal
[ ] Dapat dibantu dengan melihat korelasi prediktor - target -> bisa
[x] Hanya menggunakan prediktor berkorelasi kuat (>= 0.7) -> tidak harus, hanya disarankan. korelasi medium/rendah pun boleh dicoba.

note: walaupun variable tidak berkorelasi, namun secara bisnis harusnya berpengaruh, sarannya adalah tetap digunakan. 

3. Nilai yang lebih tepat untuk menilai kebaikan model Multiple Linear Regression adalah ...
[ ] Multiple R-squared -> simple linear regression (prediktor 1)
[x] Adjusted R-squared -> multiple linear regression

4. Terdapat ragam metrics error yang dapat dipakai dalam evaluasi model regresi. Metrics mana yang dapat diinterpretasikan dan lebih mudah dipahami khalayak umum?
[x] MAE -> bisa diinterpretasi (real value), lebih mudah dijelaskan
[ ] MSE -> tidak bisa diinterpretasi (nilai kuadrat)
[ ] RMSE -> bisa diinterpretasi (real value), lebih sulit dijelaskan 
...
pred_all_interval <- predict(model_ineq_all, crime, interval="prediction", level =0.95) # default level confidence 
pred_forward_interval <- predict(model_forward, crime, interval="prediction", level =0.95)
...
2. Uji statistik dengan `cor.test()`
...
p-value >= 0.05 = terima H0 (gagal tolak H0)
p-value < 0.05 = terima H1 (tolak H0)
...
urutannya: cek multicol dan model step (untuk pilih police_exp59 / police_exp60)
```{r}
library(car)
...
* **Asumsi Linear Model**
  ...
  3. homoscedasticity
    ...
    + uji Breusch-Pagan `bptest()`: diinginkan H0
  4 multicollienarity
