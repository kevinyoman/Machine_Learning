...
birth_ts %>% 
  ...
  #geom_point()
```
Fungsi utama decompose, yaitu mendapatkan insight dari trend dan seasonality:

> Insight:
- Trend: di awal turun, namun lambat laun makin naik. Kemungkinan besar terjadi karena efek berakhirnya Perang Dunia kedua, atau sering kita kenal sebagai Generasi Baby Boomer (generasi lahir pada tahun 1946 - 1964)
- Ada pola berulang dari data, agar lebih mudah interpretasi maka bisa di zoom in untuk melihat pergerakan setiap bulannya

```{r}
# ambil 2 tahun data untuk lihat kondisi seasonal
birth_ts %>% 
...
```
> Insight:
Seasonality: Bulan Februari tingkat kelahiran rendah, July tingkat kelahiran tinggi dibandingkan bulan-bulan lainnya.
...
> Insight: 
* persentase tingkat kelahiran paling tinggi di juli
* persentase tingkat kelahiran paling rendah di februari
...
> Insight: Berdasarkan 2 model SMA yang sudah dibangun dengan ordo 3 dan ordo 12, belum ada model yang  berhasil menangkap dengan baik pola data. Kalau secara visual diperhatikan:
- model SMA 3 pola datanya sudah dengan baik mengikuti pola data, namun pola yang terbentuk sedikit lambat dibandingkan pola data asli 
- model SMA 12 kalau diperhatikan terlalu smooth, sehingga pola naik turun tidak tertangkap dengan baik.

> Langkah lanjutan
- Bisa dilakukan evaluasi model dengan melihat nilai error antara ke-2 model
- Kalau error masih tinggi, bisa coba dituning kembali dengan ubah nilai ordo.
- Kalau tuning masih belum berhasil maka bisa dilakukan penambahan data/ganti model
...
autoplot(gas_ts)
```
> Secara umum, pola data memiliki trend yang semakin lama semakin tinggi. Kemudian terlihat bahwa data tidak memiliki seasonal/pola berulang.
...
```{r}
forecast_gas <- forecast(object = model_gas, h = 10) 
```
...
gas_ts %>% 
  autoplot() +
  autolayer(forecast_gas$mean, series = "hasil prediksi") +
...
Insight: Model `forecast_gas` memiliki error sebesar 11.108596% ketika memprediksi data test.
...
c(1987,1): artinya data kita start di bulan januari tahun 1987
freq:12 kita ingin melihat pola tahunan
...
h = 12 artinya kita akan melakukan forecasting untuk 12 bulan kedepan. Jumlah h yang kita tentukan sebanyak jumlah data test.
...
- Secara visual garis prediksi nilai test sudah mengikuti data test yang sebenarnya dengan baik
- Berdasarkan garis data aktual dan data prediksi di data train, pola data juga terlihat sudah tertangkap dengan baik
- Secara umum, model terlihat sudah baik untuk memprediksi 

8. Model evaluation menggunakan data `souvenir_test`
```{r}
# model multiplicative
# data forecast, data actual
accuracy(souvenir_forecast$mean, test)
```

> Insight: Model `souvenir_forecast` memiliki error sebesar 21.15% dalam memprediksi data test.

Evaluasi:
- Apabila asumsi MAPE yang baik adalah di sekitar 10%, maka MAPE masih cenderung besar. Maka bisa dicoba tuning model -> alpha, beta, gamma diubah.

*Note:* 
- Jadikan model HoltWinters() dengan nilai coeff yang diatur otomatis sebagai baseline model, kemudian coba naikkan atau turunkan coeff berdasarkan pehamanan terkait data kita. 
...
2. Buatlah object ts
```{r}
birth_ts <- ts(birth$births, start = c(1946,1), freq = 12)
```
...
```{r}
# a
model_birth1 <- HoltWinters(x = birth_train, seasonal = "additive")
model_birth1
# b (Pak Kevin)
model_birth2 <- HoltWinters(x = birth_train, seasonal = "additive", alpha = 0.12, beta = 0.33, gamma = 0.34)
```

7. Lakukanlah forecasting untuk 2 tahun kedepan (24 bulan)
```{r}
birth_forecast <- forecast(model_birth1, h = 24)
birth_forecast2 <- forecast(model_birth2, h = 30)
```
...
birth_train %>% 
  autoplot() +
  autolayer(model_birth2$fitted[,1]) + 
  autolayer(birth_test) +
  autolayer(birth_forecast2$mean)
...
```{r}
accuracy(birth_forecast, birth_test)
accuracy(birth_forecast2, birth_test)
```

> Insight:
- Berdasarkan nilai MAPE pada data test model `birth_forecast2` lebih baik dalam melakukan prediksi dengan error model sebesar 2.183942%

**Summary Day 3:** 

**Modelling**

1. Kita harus bisa mencocokan model sesuai data. Bagaimana cara buat model (mengisi argumen2 pada fungsi) bila ada atau tidak ada Trend/Seasonal/Error.

2. Kita boleh set auto langsung di awal. 
...
Namun penting untuk kita mengetahui apa yang dilakukan model auto tersebut. Sehingga bisa mengambil insight dari info alpha, beta, gamma. Hal ini juga bermanfaat ketika kita mau improve model dari model auto yang sudah terbentuk.

**Workflow TS**

* read data
* buat objek ts
* eda: autoplot + decompose
* cross validation
* modelling
* forecast
* evaluation
...
knitr::include_graphics("assets/stationarycomparison.png")
```
...
**Kesimpulannya:**
- Model ARIMA butuh data yang stasioner
- Kalau data tidak stasioner, maka perlu di-differencing (pakai `diff()`)
- Differencing dilakukan untuk handle prediktor yang saling berkorelasi kuat. 
...
> yang diinginkan p-value < 0.05

> Insight: data tidak stasioner
...
Insight: Setelah differencing 1x data `no_ts` sudah stasioner.

```{r eval=FALSE}
# berikut referensi bila data perlu dilakukan lebih dari 1x diff
adf.test(diff(no_ts, differences = 2))
```

4. Buat model Auto ARIMA:

```{r}
model_auto <- auto.arima(y = no_ts)

summary(model_auto)
```

Model auto arima menghasilkan MAPE 25.68% di data train.
...
tsdisplay(diff(no_ts)) # data ts yang sudah stasioner
```
...
Model yg dipakai: 
- Kondisi 1: ACF (cut off), PACF(dies down) -> `MA`
-> kita akan set ordo (p,d,q) -> Arima(0,1,1)

- Kondisi 2: ACF (cut off), PACF(cut off) -> `ARMA`
...
```{r}
# buat model
model_arima1 <- Arima(y = no_ts, order = c(0,1,1)) 
model_arima2 <- Arima(y= no_ts, order = c(1,1,1))
model_arima3 <- Arima(y= no_ts, order = c(2,1,1))
model_arima4 <- Arima(y= no_ts, order = c(4,1,1))
```
...
accuracy(model_arima4) 
accuracy(model_auto)
```

> Model terbaik berdasarkan nilai MAPE adalah `mode_arima3` dengan ordo (2,1,1)

---

### SARIMA in R

Jika data time series memiliki pola seasonal, maka kita dapat menambahkan term seasonal pada ARIMA dan menjadi **Seasonal Auto Regressive Integrated Moving Average** (SARIMA).

**Study Case: Monthly Motorcycle Sales 2011-2019**

Data penjualan motorcycle per bulan di tahun 2011-2019.
...
**Buat objek ts dengan seasonality tahunan**:

```{r}
sales_ts <- ts(data = sales_clean$DAUTONSA,
               start = c(2011,1), # mulai di tahun 2011 bulan januari
               frequency = 12) # ingin melihat pola tahunan
```
...
* differencing pada SARIMA dilakukan dengan:
  - selisih dengan lag kelipatan seasonalnya (sesuai frequency)
  - bila belum stasioner, differencing kembali dengan metode biasa `diff()`

```{r}
# diff untuk efek seasonal
adf.test(diff(sales_train, lag = 12))
```

Insight: Data belum stasioner karena masih > 0.05

```{r}
# diff efek seasonal 1x & diff biasa
sales_train %>% 
  diff(lag = 12) %>%  # diff efek seasonal
  diff(lag = 1) %>% 
  adf.test()
```

> Insight: Data sudah stasioner karena p-value < alpha (0.05)

Note: 

* Diff data seasonal biasanya 1x saja
* kalau belum stasioner, maka lanjutkan dengan diff data biasa (max 2x)
* kalau belum stasioner juga, tidak disarankan menggunakan ARIMA.

**Pembuatan model Auto SARIMA**
...
Insight: Dikeluarkan formula model **ARIMA(0,1,1)(0,1,0)[12]** 

template yang digunakan: `ARIMA(pdq)(PDQ)[frequency]`

* pdq(0,1,1): ordo untuk data secara umum
* PDQ(0,1,0): ordo untuk efek seasonal
* [12]: frequency data

**Model SARIMA manual**

Penentuan `p,d,q` dan `P,D,Q` masih sama:

* `p` & `P`: lihat plot PACF, namun untuk P pakai lag kelipatan seasonal
* `d`: lihat hasil differencing (brp kali diff seasonal dan diff biasa)
* `q` & `Q`: plot ACF, namun untuk Q pakai lag kelipatan seasonal

Cheatsheet modelnya juga masih sama:
...
```{r}
# plot ACF & PACF dengan tsdisplay 
sales_train %>% 
  diff(lag = 12) %>%  # diff efek seasonal
  diff(lag = 1) %>% 
  tsdisplay()
...
Untuk **efek seasonal** (P,D,Q), perhatikan garis lag per seasonalnya (12, 24, dst):
...
Untuk **keseluruhan data** (p,d,q), perhatikan garis lag 1 dst (seperti biasa): 


Sehingga yang cocok model **ARMA** dengan:
...
```{r}
sarima1 <- Arima(y = sales_train, order = c(1,1,1), seasonal = c(0,1,0))
sarima2 <- Arima(y = sales_train, order = c(2,1,1), seasonal = c(0,1,0))
```
...
```{r}
forecast_sarima_auto <- forecast(model_auto_sarima, h = length(sales_test))
forecast_sarima_1 <- forecast(sarima1, h = length(sales_test))
forecast_sarima_2 <- forecast(sarima2, h = length(sales_test))
```
...
```{r}
# MAPE: Mean Absolute Percentage Error
accuracy(forecast_sarima_auto, sales_test)
accuracy(forecast_sarima_1, sales_test)
accuracy(forecast_sarima_2, sales_test)
```

Insight: Model terbaik adalah `sarima2` dengan MAPE 7.19% di data test.
...
sales_train %>% 
  ...
  autolayer(sarima2$fitted, series = "fitted")+ # fitted model terpilih
  autolayer(forecast_sarima_2$mean, series = "forecast") #hasil forecast model
```
...
Insight: berdisribusi normal (uji asumsi terpenuhi)
...
Insight: Residual tidak mengalami autokorelasi (uji terpenuhi)
...
* [Function documentation](https://pkg.robjhyndman.com/forecast/reference/forecast.stl.html)
  
Metode yang kita gunakan sebelumnya untuk dekomposisi data adalah **classical decompositon**. Terdapat metode lain yang lebih robust yaitu **STL** (Seasonal and Trend Decomposition using Loess). Berikut adalah fungsi nya di R dari library `forecast`: 

* STL decompose: `mstl()`
* Modeling using STL decompose: `sltm()`
  * `y` = objek ts
  * `s.window` = seasonal window, sesuai frequency
  * `method` = model time series ("ets"/"arima")
...
```{r}
# modeling
air_stl_arima <- stlm(y = AirPassengers, # data 
                      s.window = 12, # seasonal window = frequency 
                      method = "arima") # kita bisa pilih "arima"/"ets"

# plotting forecasting result on data train
autoplot(AirPassengers) +
  autolayer(air_stl_arima$fitted)
```
Cek evaluasi error di data train:

```{r}
accuracy(air_stl_arima)
```
...
- [Tidy Time Series and Forecasting in R](https://github.com/rstudio-conf-2020/time-series-forecasting)
