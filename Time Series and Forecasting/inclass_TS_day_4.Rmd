...
library(fpp2)
data_ts <- ts(qcement, frequency = 1)
```
...
```{r}
# buat model
model_arima1 <- Arima(y = no_ts, order = c(...)) 
model_arima2 <- Arima(y=..., order = ...)
model_arima3 <- Arima(y=..., order = ...)
```
...
Sehingga yang cocok model **_....** dengan:
...
sarima2 <- Arima(y = ..., order = ...., seasonal = ....)
```
...
```{r}
forecast_sarima_auto <- forecast(..., h = ...)
forecast_sarima_1 <- forecast(..., h = ...)
```
...
Insight: Model terbaik adalah ... dengan MAPE ... di data test.
...
sales_train %>% 
  ...
  autolayer(...$fitted, series = "fitted")+ # fitted model terpilih
  autolayer(...$mean, series = "forecast") #hasil forecast model
```
